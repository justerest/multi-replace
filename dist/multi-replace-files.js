...
function multiReplaceFiles({ paths, searchValue, replaceValue }) {
    ...
    const textChanges$ = changes$.pipe(operators_1.filter(({ srcText, outText }) => srcText !== outText), rxjs_set_operators_1.mergeSet('textChanged', ({ outText, srcFilePath }) => to_boolean_1.toBoolean(fs_extra_1.writeFile(srcFilePath, outText))));
    const filePathChanges$ = changes$.pipe(operators_1.filter(({ srcFilePath, outFilePath }) => srcFilePath !== outFilePath), rxjs_set_operators_1.mergeSet('pathChanged', ({ srcFilePath, outFilePath }) => to_boolean_1.toBoolean(move_file_1.moveFile(srcFilePath, outFilePath))));
